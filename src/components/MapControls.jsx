...
export function PeriodSelector({ selectedPeriods, onTogglePeriod, onToggleAll, intervalMode, onIntervalChange, periods, stockSource, onStockSourceChange }) {
  ...
  // show the source toggle only when any selected period falls in the overlap range
  const overlapPeriods = intervalMode === "10yr" ? ["1990"] : ["1990", "1995"];
  const hasOverlap = overlapPeriods.some(p => selectedPeriods.has(p));
  ...
  return (
    <div style={{ position: "absolute", bottom: 20, left: "50%", transform: "translateX(-50%)", display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
      {/* source toggle — only visible when 1990s periods are selected */}
      {hasOverlap && (
        <div style={{ display: "flex", gap: 2, background: "#fff", padding: 3, borderRadius: 6, boxShadow: "0 1px 4px rgba(0,0,0,0.08)", border: "1px solid #e0dbd3" }}>
          <span style={{ fontSize: 9, color: "#8a857a", padding: "3px 6px", alignSelf: "center" }}>1990s source:</span>
          <button onClick={() => onStockSourceChange("wb")} style={toggle(stockSource === "wb")}>World Bank</button>
          <button onClick={() => onStockSourceChange("un")} style={toggle(stockSource === "un")}>UN 2015</button>
        </div>
      )}
